import SubscriptionProviderService from "../services/stripe-subscription";
...
class ProductVariantSubscriber {
    ...
    async onProductVariantCreated(data) {
        ...
        if (productVariant.is_subscription) {
            return await this.stripeSubscriptionService_.createProduct(productVariant)
        }
    }

    async onProductVariantUpdated(data) {
        const { id, product_id, fields } = data
        const productVariant = await this.stripeSubscriptionService_.retrieve(id, {
            select: selects,
            relations: relations,
        })

        if (productVariant.is_subscription) {
            return await this.stripeSubscriptionService_.updateProduct(productVariant)
        }
    }
    ...
    async onProductVariantDeleted(data) {
        ...
        if (productVariant.is_subscription) {
            return await this.stripeSubscriptionService_.deleteProduct(productVariant)
        }
    }
}
